...
注意：在当前版本中， `voiceIMEPatterns` 这一行必须要加，插件才能正常工作！

因为目前在记录切换到语音输入法的前一个输入法时，为了判断其自身是不是语音输入法，也会检查麦克风状态，但这会有一个时间延迟。而 Mac 系统往往在应用程序之间切换时，也会触发输入法变更的事件，于是在通过全局快捷键启动语音输入时，插件就还来不及完成对前一个输入法的判断，也就不会用它更新前一个输入法。于是在插件把输入法切换回来的时候，很可能就不是我们想要的那一个了。
